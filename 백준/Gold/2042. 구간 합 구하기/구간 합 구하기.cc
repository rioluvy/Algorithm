...
ll make_segment_tree(int node, int start, int end){
    ...
    tree_arr[node] = left + right;

    return tree_arr[node];

}

ll total(int node, int start, int end, int left, int right){
    ...
    ll left_part = total(node*2,start,mid,left,right);

    ll right_part = total(node*2+1,mid+1,end,left,right);

    return left_part + right_part;

}

void update_segment_tree(int node,int start,int end,int idx,ll diff){
...
int main(){
    ...
    for(int i = 0 ; i<m+k; i++){
        ...
        if(get<0>(info[i]) == 1){
            ...
            ll diff = get<2>(info[i]) - arr[idx];

            arr[idx] = get<2>(info[i]);
            ...
            part_sum = total(1,0,n-1,left,right);
